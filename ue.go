...
// get login code from ubereats
func (n *ImapOpts) getUberEatsCode() (string, error) {
	// connect to server
	...
	for _, box := range boxes {
		...
		for msg := range messages {
			...
			if mailsubject != "welcome to uber" {
				continue
			}
			...
			if !strings.Contains(strings.ToLower(fromaddress), "admin") && !strings.Contains(strings.ToLower(fromaddress), "uber") {
				continue
			}
